...
void main() {
  ...
  void stubSignInWithEmailAndPasswordSucceeds() {
    when(mockAuth.signInWithEmailAndPassword(any, any)).thenAnswer((_) =>
        Future<User>.value(
            User(uid: '123', photoUrl: 'url', displayName: 'NameUser')));
  }
  ...
  group('sign in', () {
    testWidgets(
      ...
      var signedId = false;
      await pumpEmailSignInForm(tester, onSignedIn: () => signedId = true);

      final signInButton = find.text('Sign in');
      await tester.tap(signInButton);

      //expect = asserts actual value vs matcher
      //verify = check if method is called on a mock
      verifyNever(mockAuth.signInWithEmailAndPassword(any, any));
      expect(signedId, false);
    });
